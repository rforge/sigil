...
## a) use RStudio package installer

## b) download appropriate file from CRAN or SIGIL page,
##    then install "from package archive file" in RStudio or on command-line with
...
## note that we also have to convert the values to floating-point format in order to avoid integer overflow
...
COLL <- transform(COLL,
                  Dice = 2 * O11 / (R1 + C1))
## (equation taken from www.collocations.de/AM)
