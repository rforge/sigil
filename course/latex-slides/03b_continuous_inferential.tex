...
qchisq(0.05, df=5, lower.tail=FALSE) \REM{one-sided test}
...
\REM{\(\Hnull: \sigma = 13\) (one-sided test against \(\sigma > \sigmaNull\))}
> sigma0 <- 13
...
\REM{Here's a trick to carry out a two-sided test (try e.g.\ with \(\sigmaNull=20\))}
...
> s2 <- sum((x - x.bar)^2) / (n-1) \REM{sample variance \(\sSq\)}
> s2 <- sd(x)^2 \REM{easier with built-in function (check equality!)}
> t.score <- (x.bar - mu0) / sqrt(s^2 / n) \REM{\(t\) statistic}
